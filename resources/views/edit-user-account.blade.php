                    ...
                    <div class="card h-100 w-100 mt-n4 bg-gradient-dark">
                        ...
                        <div class="card-body">
                            <form action="{{ route('user-account.updateUser', $user->id) }}" method="POST"
                                ...
                                <div class="form-group">
                                    <label for="pos" class="form-control-label text-light">Pos</label><br>
                                    <select class="form-select" name="pos">
                                        <option value="">---Pilih Pos---</option>
                                        <option value="marking" @if ($user->pos === 'marking') selected @endif>Marking
                                        </option>
                                        <option value="dolly-tool-supply" @if ($user->pos === 'dolly-tool-supply') selected @endif>
                                            Dolly Tool Supply</option>
                                        <option value="tools-input" @if ($user->pos === 'tools-input') selected @endif>Tools
                                            Input</option>
                                        <option value="disasemmbly" @if ($user->pos === 'disasemmbly') selected @endif>
                                            Disasemmbly</option>
                                        <option value="washing" @if ($user->pos === 'washing') selected @endif>Washing
                                        </option>
                                        <option value="regrinding-auto" @if ($user->pos === 'regrinding-auto') selected @endif>
                                            Regrinding Auto</option>
                                        <option value="regrinding-manual" @if ($user->pos === 'regrinding-manual') selected @endif>
                                            Regrinding Manual</option>
                                        <option value="pre-assembly" @if ($user->pos === 'pre-assembly') selected @endif>
                                            Pre-Assembly</option>
                                        <option value="setting-tool-mc-nt"
                                            @if ($user->pos === 'setting-tool-mc-nt') selected @endif>Setting Tool Mc NT</option>
                                        <option value="setting-tool-mc-spe"
                                            @if ($user->pos === 'setting-tool-mc-spe') selected @endif>Setting Tool Mc Spe</option>
                                        <option value="setting-tool-mc-zol"
                                            @if ($user->pos === 'setting-tool-mc-zol') selected @endif>Setting Tool Mc Zol</option>
                                    </select>
